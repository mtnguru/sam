...
The glossary_old module uses a filter that scans posts for glossary_old terms 
(including synonyms). The glossary_old indicator is inserted after every
...
that term presented within the whole glossary_old or directly to the
detailed description of the term, if available.

The glossary_old uses Drupal's built in taxonomy feature, so you can organize
...
the taxonomy terms in the glossary_old vocabulary. Descriptions are used to
...
to allow you to display an image for each term in the glossary_old.
...
2. Glossary terms are managed as vocabularies within the taxonomy.module.
   To get started with glossary_old, create a new vocabulary on the
   ...
   The term title should be the glossary_old entry, the description should be
   ...
   of the glossary_old when viewed in an overview. Synonyms will be flaggged in
...
5. If you want a search box on your glossary_old page, enable the block on admin/block page.


Advanced Usage
--------------

You can create a dedicated glossary_old for some pages of your site. To do so, create a new
vocabulary and put the special terms in it. Then create a new input format and add the
glossary_old filter to it. Then configure glossary_old filter in that format to look at the new
vocabulary. Finally, affiliate the special pages with this input format and you will get
the desired behavior.

You may also set up a "dictionary" that looks just like the glossary_old page, but is not 
...
Modified extensively by Al Maw <drupal-glossary_old [at] almaw.com>.
